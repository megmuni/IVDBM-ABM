 ...
 * each tick of the simulation. Calculates cell insertion and deletion
 * statistics as well as execution time information.
 *
 * Return: 0 if succesful
 *
...
/* -------------------------------------------------------------------------- */
  ...
  for (int tick = 0; tick < numTicks; tick++) {
    ...
    elapsed_times[tick] =
        ...
        (start.tv_sec * 1000 +
         start.tv_usec / 1000); // Store execution time for this tick
    // cout << "	this go() execution took " << elapsed_times[tick] << "
    // ms" << endl;
  }
  ...
  for (int i = 0; i < numTicks; i++) {
    cout << "		tick " << i << ": " << endl;
    cout << "		# Added Chonds:	   " << myWorld.numAddedChondros[i]
         << endl;
    cout << myWorld.numAddedCells[i] << endl;
    addedSum += myWorld.numAddedCells[i];
    addedMax = myWorld.numAddedCells[i] > addedMax ? myWorld.numAddedCells[i]
                                                   : addedMax;
    addedMin = myWorld.numAddedCells[i] < addedMin ? myWorld.numAddedCells[i]
                                                   : addedMin;
    addedMin_NoZero =
        (myWorld.numAddedCells[i] < addedMin) && (myWorld.numAddedCells[i] > 0)
            ? myWorld.numAddedCells[i]
            : addedMin;
  }
  ...
  for (int i = 0; i < numTicks; i++) {
    cout << "		tick " << i << ": " << endl;
    cout << "	    # Erased Chonds:	" << myWorld.numErasedChondros[i]
         << endl;
    cout << myWorld.numErasedCells[i] << endl;
    erasedSum += myWorld.numErasedCells[i];
    erasedMax = myWorld.numErasedCells[i] > addedMax ? myWorld.numErasedCells[i]
                                                     : addedMax;
    erasedMin = myWorld.numErasedCells[i] < addedMin ? myWorld.numErasedCells[i]
                                                     : addedMin;
    erasedMin_NoZero = (myWorld.numErasedCells[i] < addedMin) &&
                               (myWorld.numErasedCells[i] > 0)
                           ? myWorld.numErasedCells[i]
                           : addedMin;
  }
  ...
  util::writeRunTimingJson(acc, numTicks,
                           (double)(clock() - tStart) / CLOCKS_PER_SEC);
