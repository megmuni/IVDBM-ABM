 ...
 * @brief Place random chemical concentration spikes on a patch grid and optionally
 *        run diffusion ticks (Diffusion3D via ChemicalEnvironment).
 *
 * Run from the project root so default config paths resolve, e.g.:
 *   ./build/bin/ex_chem_random_spikes --nx 32 --ny 32 --nz 32 --spikes 25 --ticks 5
 * Defaults use low diffusivity scale (0.01) so spikes spread slowly on a 32^3 grid.
 *
 * ParaView: File → Open → output/chem_spikes/chem_spikes_t*.vti → group as time series.
 */
...
struct Options
{
    ...
    /** Scales D_eff = base_diffusivity * scale (demo default << 1 for slower spread). */
    ...
    /** -1 = patch_field default; else force ExplicitHeatEquation / GpuStencil / GpuFftPrecomputed. */
...
void usage(const char *prog)
{
    std::cerr
        ...
        << "  --spike-min V       Min spike amount (default 0.1, typical production < 10)\n"
        << "  --spike-max V       Max spike amount (default 10)\n"
        << "  --seed N            RNG seed (default: random_device)\n"
        << "  --species NAME      TNF | TGF | IL1beta (default: random per spike)\n"
        << "  --chem-config PATH  chemical_environment.json\n"
        << "  --out-dir PATH      VTK output directory (default output/chem_spikes)\n"
        << "  --no-paraview       Skip VTK (.vti) export\n"
        << "  --viz-multiplier M  Extra ParaView array *= M when M != 1 (default 1)\n"
        << "  --diffusivity-scale S  D_eff = base_D * S (default 0.01, slower spread)\n"
        << "  --tick-dt MIN       Diffusion step length in minutes (default: JSON)\n"
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (arg == "--algorithm")
        {
            ...
            if (strcmp(v, "cpu") == 0 || strcmp(v, "explicit") == 0)
                opt.algorithm_choice = static_cast<int>(DiffusionAlgorithm::ExplicitHeatEquation);
                ...
                opt.algorithm_choice = static_cast<int>(DiffusionAlgorithm::GpuFftPrecomputed);
            else
            {
                std::cerr << "Unknown algorithm: " << v << " (use cpu, stencil, or fft)\n";
                return false;
            }
        }
        else
                 ...
                 const char *label)
{
    ...
    for (SpeciesId sp : {TNF, TGF, IL1beta})
    {
        ...
        std::cout << "  " << species_name(sp) << "  max=" << st.max_c
                  << " at (" << st.max_ix << "," << st.max_iy << ","
                  << st.max_iz << ")"
                  << "  sum=" << st.sum_c << "\n";
    }
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        const double tick_dt =
            opt.tick_dt_minutes > 0.0 ? opt.tick_dt_minutes
        ...
        if (opt.write_paraview)
        {
            std::cout << "Open in ParaView: File → Open → "
                      << opt.output_dir << "/chem_spikes_t*.vti"
