...
Biological values for the chemistry system are read only from JSON. The C++ code does not embed diffusivities, baselines, or tick length.
...
3. Run from the project root (default path), or pass a custom file:
   ...
   ./build/bin/testRun \
     ...
     --chem-config /path/to/chemical_environment.json
   ```
...
Not used for chemistry after JSON load: `Baseline_TNF`, `Baseline_TGF`, `Baseline_IL1beta`, and legacy `D_*` / `HL` tags in the text file. Set those in `chemical_environment.json` instead.
