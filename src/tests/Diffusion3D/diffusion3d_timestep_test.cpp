 ...
 * @brief Catch2 tests for diffusion3d_timestep (stability constraint +
 * plan_substeps).
 */
...
TEST_CASE("compute_stability_constraint matches h^2/(6D) with safety",
          "[timestep]") {
  const double h = 0.5;
  const double D = 0.12;
...
TEST_CASE("compute_stability_constraint is infinite when D is zero",
          "[timestep]") {
...
TEST_CASE("plan_substeps tick_dt zero yields one zero-length substep",
          "[timestep]") {
...
TEST_CASE("plan_substeps with infinite dt_max uses single step", "[timestep]") {
  const SubstepPlan p =
      plan_substeps(2.5, std::numeric_limits<double>::infinity());
  CHECK(p.n_sub == 1);
  CHECK(p.dt_sub == Approx(2.5));
}

TEST_CASE("plan_substeps ceil splits tick into substeps bounded by dt_max",
          "[timestep]") {
...
TEST_CASE("stencil centre weight stays positive for every D at safety < 1",
          "[timestep]") {
  const double h = 0.01;
