...
bool nearly_equal_vec(const std::vector<double> &a,
  ...
  for (int z = 0; z < g.nz_; ++z)
    for (int y = 0; y < g.ny_; ++y)
      for (int x = 0; x < g.nx_; ++x)
        g.at(x, y, z) =
            0.01 * static_cast<double>(x + y * g.nx_ + z * g.nx_ * g.ny_);
}

MultiSpeciesDiffusionSettings make_settings(SpeciesId id, double D,
                                            double safety = 0.5) {
...
TEST_CASE("MultiSpeciesFieldGrid holds per-species grids on shared domain",
          "[core][field_grid]") {
...
TEST_CASE("Single-species stepper matches explicit scalar euler micro-steps",
          "[core][engine]") {
  ...
  const SubstepPlan plan =
      plan_substeps(tick_dt, compute_stability_constraint(h, D, 0.5));
...
TEST_CASE("Multi-species fields evolve independently",
          "[core][engine][multi]") {
  ...
  CHECK_FALSE(
      nearly_equal_vec(grid.grid(slow).data_, grid.grid(fast).data_, 1e-12));
}

TEST_CASE("validate rejects safety on or past the stability edge",
          "[core][settings]") {
...
TEST_CASE("Engine tracks the analytic Gaussian at biological scale",
          "[core][engine][analytic]") {
  const int n = 81;
  const double h = 0.01;
...
TEST_CASE("MakeMultiSpeciesDiffusionEngine creates CPU stepper",
          "[core][factory]") {
  MultiSpeciesDiffusionSettings settings = make_settings(0, 0.5);
  auto engine = MakeMultiSpeciesDiffusionEngine(settings);
  REQUIRE(engine != nullptr);
  CHECK(engine->resolved_algorithm() ==
        DiffusionAlgorithm::ExplicitHeatEquation);
}

TEST_CASE("configure_species_interval requires tick_dt macro interval",
          "[core][engine]") {
...
TEST_CASE("GpuStencil stepper matches CPU stepper for single species",
          "[core][gpu]") {
  ...
  CHECK(nearly_equal_vec(cpu_grid.grid(species).data_,
                         gpu_grid.grid(species).data_, 1e-12));
}

TEST_CASE("MakeMultiSpeciesDiffusionEngine creates GpuStencil stepper",
          "[core][factory][gpu]") {
  MultiSpeciesDiffusionSettings settings = make_settings(0, 0.5);
