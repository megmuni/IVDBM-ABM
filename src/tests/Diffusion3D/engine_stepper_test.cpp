 ...
 * @brief Catch2 tests for MultiSpeciesDiffusionEngine steppers (replaces
 * DiffusionSolver suite).
 */
...
TEST_CASE("ExplicitMultiSpeciesHeatStepper splits tick into stable substeps",
          "[engine][cpu]") {
  ...
  MultiSpeciesDiffusionSettings settings = make_settings(
      species, D, DiffusionAlgorithm::ExplicitHeatEquation, safety);
...
TEST_CASE("MakeMultiSpeciesDiffusionEngine resolves algorithm per CUDA build",
          "[engine][backend]") {
  MultiSpeciesDiffusionSettings settings =
      make_settings(0, 0.5, DiffusionAlgorithm::GpuFftPrecomputed);
...
TEST_CASE("ExplicitGpuFftStepper matches CPU stepper including boundaries",
          "[engine][gpu][fft]") {
  ...
  MultiSpeciesDiffusionSettings cpu_settings =
      make_settings(species, D, DiffusionAlgorithm::ExplicitHeatEquation);
  MultiSpeciesDiffusionSettings fft_settings =
      make_settings(species, D, DiffusionAlgorithm::GpuFftPrecomputed);
