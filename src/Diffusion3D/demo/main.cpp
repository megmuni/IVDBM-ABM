/**
 * @file main.cpp
 * @brief ParaView demo: CPU stencil, GPU stencil, and GPU FFT via MultiSpeciesDiffusionEngine.
 *
 ...
 * Open in ParaView: File → Open → select diffusion3d_t*.vti → group as time series.
 */
...
void log_substep_plan(const char *label, double h, double D, double tick_dt, double safety)
{
...
void run_time_series(
    const char *label,
    const std::string &outdir,
    DiffusionAlgorithm algo,
    int nx, int ny, int nz,
    double h, double D, double tick_dt,
    int nsteps)
{
    std::error_code ec;
    ...
    std::cout << "demo [" << label << "] " << algorithm_label(engine->resolved_algorithm()) << "\n";
    log_substep_plan(label, h, D, tick_dt, settings.safety);

    if (auto *cpu = dynamic_cast<ExplicitMultiSpeciesHeatStepper *>(engine.get()))
    {
        assert(cpu->n_sub(kSpecies) > 1 && "pick tick_dt/h/D that triggers substepping");
    }

    for (int t = 0; t <= nsteps; ++t)
    {
        char path[256];
        std::snprintf(path, sizeof(path), "%s/diffusion3d_t%d.vti", outdir.c_str(), t);
        export_scalar_field_to_vti(grid.grid(kSpecies), h, path, "u");
        if (t < nsteps)
            engine->advance_species_interval(grid);
    }

    std::cout << "demo [" << label << "] wrote " << (nsteps + 1)
              << " frames to " << outdir << "/\n";
}

#ifdef DIFFUSION3D_CUDA
void export_cpu_gpu_stencil_compare(
    int nx, int ny, int nz, double h, double D, double tick_dt, int nsteps)
{
    ...
    MultiSpeciesDiffusionSettings cpu_settings = make_settings(DiffusionAlgorithm::ExplicitHeatEquation, D);
    MultiSpeciesDiffusionSettings gpu_settings = make_settings(DiffusionAlgorithm::GpuStencil, D);
    ...
    export_cpu_gpu_compare_to_vti(
        cpu_grid.grid(kSpecies).data_,
        gpu_grid.grid(kSpecies).data_,
        nx, ny, nz, h,
        outdir + "/cpu_vs_gpu_stencil_final.vti");

    std::cout << "demo [compare] wrote " << outdir << "/cpu_vs_gpu_stencil_final.vti\n";
}
#endif
...
int main()
{
    ...
    run_time_series(
        "cpu",
        std::string(kOutputRoot) + "/cpu_series",
        DiffusionAlgorithm::ExplicitHeatEquation,
        nx, ny, nz, h, D, tick_dt, nsteps);

#ifdef DIFFUSION3D_CUDA
    run_time_series(
        "gpu_stencil",
        std::string(kOutputRoot) + "/gpu_series",
        DiffusionAlgorithm::GpuStencil,
        nx, ny, nz, h, D, tick_dt, nsteps);

    run_time_series(
        "gpu_fft",
        std::string(kOutputRoot) + "/fft_series",
        DiffusionAlgorithm::GpuFftPrecomputed,
        nx, ny, nz, h, D, tick_dt, nsteps);

    export_cpu_gpu_stencil_compare(nx, ny, nz, h, D, tick_dt, nsteps);
#else
    std::cout << "demo: GPU paths skipped — rebuild with -DDIFFUSION3D_CUDA=ON for "
