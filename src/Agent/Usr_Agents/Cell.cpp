...
float Cell::cytokineSynthesis[10] = {
    10, 0.05, 10, 5, 2.4, 4, 2, 5, 1, 3.2}; // calibration variables
...
float Stem::collagenSynthRate =
    1; // placeholder values which will be recalculated
float Stem::aggrecanSynthRate =
    0.5; // placeholder values which will be recalculated
...
float Stem::differentiation[4] = {0.7, 0.5, 0.001, 48};
...
// float Progen::proliferation[1] = {24}; // values are the same as Cell/Stem;
// can just use the equivalent params defined in Stem float
// Progen::differentiation[3] = {0.7, 0.3, 48}; // values are the same as Stem;
// can just use the equivalent params defined in Stem
...
Stem::Stem() { cout << "default stem alloc" << endl; }

Progen::Progen() { cout << "default pre-np alloc" << endl; }

NP::NP() { cout << "default np alloc" << endl; }
...
/* In OMP version, we wait to add cells at the end of the tick, whereas in
 * serial version, cells are always added right away. Thus, cells, when added in
 * OMP, should be alive right away. */
#ifdef _OMP
...
NP::NP(Patch *patchPtr) : Cell(patchPtr) {
  ...
  if (Agent::agentWorldPtr->clock == 0)
    this->life[write_t] = BMWorld::reportTick(0, rand() % 12);
  else
...
Cell::Cell(int x, int y, int z) {
  ...
  // #ifndef MODEL_SCAFFOLD
  //	// Unactivated chondrocytes live for 5 to 11 days. 0 corresponds to
  //hours. 	if (Agent::agentWorldPtr->clock == 0) this->life[write_t] =
  //BMWorld::reportTick(0, rand()%12); 	else this->life[write_t] =
  //BMWorld::reportTick(0, 5 + rand()%7); #else 	this->life[write_t] =
  //BMWorld::reportTick(0, 5 + rand()%7); #endif
  ...
  /* In OMP version, we wait to add cells at the end of the tick, whereas in
   * serial version, cells are always added right away. Thus, cells, when added
   * in OMP, should be alive right away. */
#ifdef _OMP
  this->ix[read_t] = x;
  this->iy[read_t] = y;
  this->iz[read_t] = z;
...
void Cell::cellFunction() {
  ...
  if (this->alive[read_t] == true && (meanO2 + patchO2) > this->get_OCR()) {
    ...
    float oxDecrease = this->get_OCR() /
                       27; // divide OCR across 27 patches (current + neighbors)
...
void Cell::cellSniff() {
  ...
  if ((Agent::agentPatchPtr[in]).inDamzone == true) {
    if (rollDice(80) && this->moveTowardChemotaxis() == true) {
      ...
      if (speed > 1 && Agent::agentPatchPtr[in].type[read_t] == CaAlg) {
        ...
        for (int dx = 0; dx < speed; dx++)
          this->wiggle();
      }
      // If cell is not actively migrating, consider chance of moving to next
      // patch:
  ...
  // TGF can excite NP cell and overcome gradient: //NOTE MM: double check this
  // for MSCs
  if (can_tgf_excite()) {
#ifdef MODEL_SCAFFOLD
    if (speed > 1 && Agent::agentPatchPtr[in].type[read_t] == CaAlg) {
      for (int dx = 0; dx < speed; dx++)
        this->wiggle(); // Move up to "migrationSpeed" patches per tick:
    } else if (rollDice(0.25)) {
      this->wiggle(); // If cell is not actively migrating, consider chance of
                      // moving to next patch:
    }
#else
      this->wiggle();
#endif
...
void Cell::copyAndInitialize(Agent *original, int dx, int dy, int dz) {
  ...
  this->index[write_t] = this->ix[write_t] + this->iy[write_t] * Agent::nx +
                         this->iz[write_t] * Agent::nx * Agent::ny;
  ...
  if (Agent::agentPatchPtr[in].isOccupied() == false) {
    ...
    cout << "error in hatching and initialization!!!" << dx << " " << dy
         << endl;
  }
}
...
void Cell::proliferate() {
  int in = this->index[read_t];
  if (!(Agent::agentPatchPtr[in].type[read_t] == CaAlg))
    return; // check for being on a biomaterial patch
  if (!(this->life[read_t] > 0 &&
        this->life[read_t] % static_cast<int>(Cell::proliferation[0]) == 0))
    return; // check for 24-hour mark (of the cell's life) to try division
  if (!isProliferative())
    return; // check if cell is proliferative; i.e., under the max # of
            // divisions for its type
  ...
  float prob = get_prolif_prob(
      meanTGF, meanIL1,
      meanTNF); // get the proliferation probability for the cell type
...
void Cell::differentiate() {
  ...
  if (!(Agent::agentPatchPtr[in].type[read_t] == CaAlg))
    return; // check for being on a biomaterial patch
  if (!(this->life[read_t] > 0 &&
        this->life[read_t] % static_cast<int>(Stem::differentiation[3]) == 0))
    return; // check for 48-hour mark (of the cell's life) to try
            // differentiation
  if (!isProliferative())
    return;
  ...
  if (daughterType == -1)
    return; // base Cell has no daughter type; skip

  if (rollDice(prob)) {
    if (rollDice(Stem::differentiation[0] *
                 100)) { // check for asymmetric differentiation; more likely
      ...
      this->hatchnewcell(1, daughterType,
                         1); // 'change' cell here to next cell stage
      this->doublings[write_t] = this->doublings[read_t] + 1;
      this->hatchnewcell(1,
                         daughterType); // create new cell in next stage nearby
...
void Cell::ecm_synthesis() {
  ...
  // Calculate total volume of surrounding patches to check for cytokine
  // thresholds
...
void Cell::calculate_ecm_synth_rates(float meanTGF, float meanIL1,
                                     float meanTNF, float patchesVolume) {}
...
void Cell::makeOCollagen(float meanTGF, float meanIL1) {
  ...
  if (neighbors.size() > 0) {
    int tid = 0;
#ifdef _OMP
    tid = omp_get_thread_num(); // Get thread id in order to access the seed
    ...
    Agent::agentECMPtr[in].ocollagen[write_t] =
        Agent::agentECMPtr[in].ocollagen[read_t] + 1 + rand() % 2;
#ifdef OPT_ECM
...
// Make a list of neighboring patches
  ...
  if (neighbors.size() > 0) {
    ...
    Agent::agentECMPtr[in].oaggrecan[write_t] =
        Agent::agentECMPtr[in].oaggrecan[read_t] + 1 + rand() % 2;
#ifdef OPT_ECM
    Agent::agentECMPtr[in].set_dirty();
#endif
  }
...
void Cell::create_cytokines(float patchTGF, float patchIL1beta,
                            float patchTNF) {}
...
float Cell::get_prolif_prob(float meanTGF, float meanIL1, float meanTNF) {
  return 10;
} // base default

int Cell::get_daughter_type() { return -1; } // sentinel 'no type'

float Cell::get_diff_prob(float meanTGF, float meanIL1, float meanTNF) {
  return 5;
} // base default
...
void Cell::hatchnewcell(int number, int agentType, int here) {
  ...
  for (int i = 0; i < 27 && newcells < number; i++) {
...
#endif
    if (here ==
        0) { // Default option; hatching on number of neighboring patches
    ...
    /* If executing OMP version, add the pointer to this new cell to the
     * thread-local list first. BMWorld::UpdateCells() will take care of putting
     * it in the global list at the end
     */
#ifdef _OMP
    int tid = omp_get_thread_num();
    Agent::agentWorldPtr->localNewCells[tid]->push_back(newcell);
#else
      // If executing serial version, add the pointer to this new cell to the
      // global list right away
...
float Stem::get_prolif_prob(float meanTGF, float meanIL1, float meanTNF) {
...
#ifdef CALIBRATION
  // float prolif = log10(1 - Stem::proliferation[1] * meanTNF -
  // Stem::proliferation[2] * meanIL1 + TGFrelated * meanTGF -
  // Stem::proliferation[3] * BMWorld::E);
...
float Stem::get_diff_prob(float meanTGF, float meanIL1, float meanTNF) {

  // return (Stem::differentiation[1]*100) + (Stem::differentiation[2] *
  // meanTGF);
  return 20;
}

void Stem::calculate_ecm_synth_rates(float meanTGF, float meanIL1,
                                     float meanTNF, float patchesVolume) {
#ifdef CALIBRATION
  Stem::collagenSynthRate =
      Stem::CollagenSynth[0] + (log10(1 + meanTGF) / (1 + meanTNF + meanIL1));
...
#else
    Stem::collagenSynthRate =
        10 + (log10(1 + meanTGF) / (1 + meanTNF + meanIL1));
...
void Stem::create_cytokines(float patchTGF, float patchIL1beta,
                            float patchTNF) {
  int in = this->index[read_t];
  // Change in chemicals due to cells:
#ifdef CALIBRATION
  this->addPatchChemSecretion(
      TGF, in,
      Stem::cytokineSynthesis[0] + Cell::cytokineSynthesis[1] * (patchTGF) +
          Cell::cytokineSynthesis[2] * (patchIL1beta) +
          Cell::cytokineSynthesis[3] *
              (patchTNF)); // this->addPatchChemSecretion(TGF, in,
                           // Chondrocyte::cytokineSynthesis[0] +
                           // Chondrocyte::cytokineSynthesis[1]*(1 +
                           // Chondrocyte::cytokineSynthesis[2]*patchTNF);
  // this->addPatchChemSecretion(TGF, in, 0; //DEBUG : constant TGF
  this->addPatchChemSecretion(
      TNF, in,
      Stem::cytokineSynthesis[1] +
          (Cell::cytokineSynthesis[5] *
           ((patchIL1beta) /
            (1 +
             Cell::cytokineSynthesis[6] *
                 patchTGF)))); // this->addPatchChemSecretion(TNF, in,
                               // Chondrocyte::cytokineSynthesis[3] +
                               // Chondrocyte::cytokineSynthesis[4]/(1 +
                               // patchTGF*Chondrocyte::cytokineSynthesis[5]);
  // this->addPatchChemSecretion(TNF, in, 0; //DEBUG : constant TNF
  this->addPatchChemSecretion(
      IL1beta, in,
      Stem::cytokineSynthesis[2] +
          (Cell::cytokineSynthesis[8] *
           ((patchTNF) /
            (1 +
             Cell::cytokineSynthesis[9] *
                 patchTGF)))); // this->addPatchChemSecretion(IL1beta, in,
                               // Chondrocyte::cytokineSynthesis[6] +
                               // (Chondrocyte::cytokineSynthesis[7]*patchTNF)/(Chondrocyte::cytokineSynthesis[8]
                               // + Chondrocyte::cytokineSynthesis[9]*patchTGF);
#else
    this->addPatchChemSecretion(
        TGF, in,
        5 + (2.25 * patchTGF + 1.3 * patchIL1beta +
             5.11 * patchTNF)); // 9.98 + 2.58*patchTGF + 5.11*patchTNF;
                                // //2.11 + 3.7*patchTGF;
    this->addPatchChemSecretion(
        TNF, in,
        0 + (2.42 * patchIL1beta) /
                (1 +
                 4.22 *
                     patchTGF)); // 5.16 + (2.42*patchIL1beta)/(1
                                 // + 4.22*patchTGF);	//2.4*patchIL1beta
                                 // + 4.8/(1 + 1.27*patchTGF);
    this->addPatchChemSecretion(
        IL1beta, in,
        0 + (5.43 * patchTNF) /
                (1 + 3.26 * patchTGF)); // 2.11 + (5.43*patchTNF)/(1
                                        // + 3.26*patchTGF);		//4;
#endif
}
...
float Stem::get_migration_speed() {
  if (BMWorld::clock == 0) {
#ifdef CALIBRATION
    float migration_ummin = Stem::CaAlgMigration[0] * log(BMWorld::E) +
                            Stem::CaAlgMigration[1]; // um/min

    if (rollDice(
            0.5)) { // Convert migration speed in um/min to patches/tick where
                    // default patchlength is 10um and default tick is 30 min
      Stem::migrationSpeed =
          ceil(migration_ummin * 30 /
               (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
    } else {
      Stem::migrationSpeed =
          floor(migration_ummin * 30 /
                (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
    }
#else
      float migration_ummin =
          0.1096 * log(BMWorld::E) +
          0.35; // um/min //float migration_ummin =
                // 0.1213*log10(Agent::agentWorldPtr->E) + 0.223; // um/min

      if (rollDice(
              0.5)) { // Convert migration speed in um/min to patches/tick where
                      // default patchlength is 10um and default tick is 30 min
        Stem::migrationSpeed = ceil(
            migration_ummin * 30 /
            (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
      } else {
        Stem::migrationSpeed = floor(
            migration_ummin * 30 /
            (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
      }
#endif
    cout << "        Stem cell migration Speed (patch/tick) = "
         << Stem::migrationSpeed << endl;
  }
  return Stem::migrationSpeed;
}

float Stem::get_apoptosis_chance() { return Stem::apoptosisChance; }

float Stem::get_OCR() { return Stem::OCR; }
...
float Progen::get_prolif_prob(float meanTGF, float meanIL1, float meanTNF) {
...
float Progen::get_diff_prob(float meanTGF, float meanIL1, float meanTNF) {

  return 20;
}

void Progen::calculate_ecm_synth_rates(float meanTGF, float meanIL1,
                                       float meanTNF, float patchesVolume) {
#ifdef CALIBRATION
  Progen::aggrecanSynthRate =
      Progen::AggrecanSynth[0] + (log10(1 + meanTGF) / (1 + meanTNF + meanIL1));
#else
...
void Progen::create_cytokines(float patchTGF, float patchIL1beta,
                              float patchTNF) {
  int in = this->index[read_t];
  // Change in chemicals due to cells:
#ifdef CALIBRATION
  this->addPatchChemSecretion(
      TGF, in,
      Progen::cytokineSynthesis[0] + Cell::cytokineSynthesis[1] * (patchTGF) +
          Cell::cytokineSynthesis[2] * (patchIL1beta) +
          Cell::cytokineSynthesis[3] *
              (patchTNF)); // this->addPatchChemSecretion(TGF, in,
                           // Chondrocyte::cytokineSynthesis[0] +
  ...
  this->addPatchChemSecretion(
    ...
    this->addPatchChemSecretion(
        TGF, in,
        1 + (0.25 * patchTGF + 1.3 * patchIL1beta +
             5.11 * patchTNF)); // 9.98 + 2.58*patchTGF + 5.11*patchTNF;
                                // //2.11 + 3.7*patchTGF;
    this->addPatchChemSecretion(
        TNF, in,
...
float Progen::get_migration_speed() {
  if (BMWorld::clock == 0) {
#ifdef CALIBRATION
    float migration_ummin = Progen::CaAlgMigration[0] * log(BMWorld::E) +
                            Progen::CaAlgMigration[1]; // um/min

    if (rollDice(
            0.5)) { // Convert migration speed in um/min to patches/tick where
                    // default patchlength is 10um and default tick is 30 min
      Progen::migrationSpeed =
          ceil(migration_ummin * 30 /
               (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
    } else {
      Progen::migrationSpeed =
      ...
      float migration_ummin =
          0.1096 * log(BMWorld::E) +
          ((NP::migrationSpeed - Stem::migrationSpeed) /
           2); // um/min //float migration_ummin =
               // 0.1213*log10(Agent::agentWorldPtr->E) + 0.223; // um/min

      if (rollDice(
              0.5)) { // Convert migration speed in um/min to patches/tick where
                      // default patchlength is 10um and default tick is 30 min
        Progen::migrationSpeed = ceil(
            migration_ummin * 30 /
            (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
      } else {
        Progen::migrationSpeed = floor(
            migration_ummin * 30 /
            (Agent::agentWorldPtr->patchlength * pow(10, 3))); // patch/tick
      }
#endif
    cout << "        Progenitor cell migration Speed (patch/tick) = "
         << Progen::migrationSpeed << endl;
  }
  return Progen::migrationSpeed;
}

float Progen::get_apoptosis_chance() { return Progen::apoptosisChance; }

float Progen::get_OCR() { return Progen::OCR; }
...
float NP::get_prolif_prob(float meanTGF, float meanIL1, float meanTNF) {
...
#endif
    TGFrelated =
        1; // Low TGF (0.1-1 ng) stimulate chond proliferation and attraction.
  } else {
    TGFrelated = -1; // High TGF (1-10 ng) inhibits proliferation.
  }

#ifdef CALIBRATION
  float prolif = Cell::proliferation[2] *
                     (log10(1 + meanTNF + meanIL1 + TGFrelated * meanTGF)) +
...
void NP::calculate_ecm_synth_rates(float meanTGF, float meanIL1, float meanTNF,
                                   float patchesVolume) {
#ifdef CALIBRATION
  NP::collagenSynthRate =
      NP::CollagenSynth[0] *
      (NP::CollagenSynth[1] * BMWorld::reportDay() + NP::CollagenSynth[2]);
  NP::aggrecanSynthRate =
      NP::AggrecanSynth[0] *
...
void NP::create_ecm(float meanTGF, float meanIL1, float meanTNF) {
  // Active cell adhered to Ca-Alg synthesis ECM according the substrate
...
void NP::create_cytokines(float patchTGF, float patchIL1beta, float patchTNF) {
  ...
  this->addPatchChemSecretion(
      TGF, in,
      Cell::cytokineSynthesis[0] + Cell::cytokineSynthesis[1] * (patchTGF) +
          Cell::cytokineSynthesis[2] * (patchIL1beta) +
          Cell::cytokineSynthesis[3] *
              (patchTNF)); // this->addPatchChemSecretion(TGF, in,
                           // Chondrocyte::cytokineSynthesis[0] +
  ...
  this->addPatchChemSecretion(
    ...
    this->addPatchChemSecretion(
        TGF, in,
        10 + 0.05 * (patchTGF +
                     10 * patchTNF)); // 9.98 + 2.58*patchTGF + 5.11*patchTNF;
                                      // //2.11 + 3.7*patchTGF;
    this->addPatchChemSecretion(
        TNF, in,
        5 + (2.4 * patchIL1beta) /
                (1 +
                 4 * patchTGF)); // 5.16 + (2.42*patchIL1beta)/(1
                                 // + 4.22*patchTGF);	//2.4*patchIL1beta
                                 // + 4.8/(1 + 1.27*patchTGF);
    this->addPatchChemSecretion(
        IL1beta, in,
        2 + (5 * patchTNF) /
                (1 + 3.2 * patchTGF)); // 2.11 + (5.43*patchTNF)/(1
                                       // + 3.26*patchTGF);		//4;
#endif
}
...
float NP::get_migration_speed() {
#ifdef CALIBRATION
  float migration_ummin =
      NP::CaAlgMigration[0] * log(BMWorld::E) + NP::CaAlgMigration[1]; // um/min
  ...
  if (rollDice(
    ...
    float migration_ummin =
        0.1096 * log(BMWorld::E) +
        0.2431; // um/min //float migration_ummin =
                // 0.1213*log10(Agent::agentWorldPtr->E) + 0.223; // um/min
...
#endif
  cout << "        NP cell migration Speed (patch/tick) = "
       << NP::migrationSpeed << endl;
  return NP::migrationSpeed;
}

bool NP::can_tgf_excite() { return this->meanNeighborConcentration(TGF) > 0; }

float NP::get_apoptosis_chance() { return NP::apoptosisChance; }

float NP::get_OCR() { return NP::OCR; }